package com.shivanthah.grpc.lb.eureka;
...
public class EurekaNameResolver extends NameResolver {
    ...
    private void update(Listener listener) {
        ...
        List<EquivalentAddressGroup> resolvedServerInfos = application.getInstances().stream().map(instanceInfo -> {
            ...
            sockaddrs_list.add(new InetSocketAddress(instanceInfo.getHostName(), port));
            return new EquivalentAddressGroup(sockaddrs_list);
